...
// import { FileUpload } from "@/components/ui/file-upload";
// import axios from "axios";
// import { loadStripe } from "@stripe/stripe-js"; // Import Stripe for handling payments
// import "./style.css";
...
//     const authToken = localStorage.getItem("authToken");
//     const amount = 50; // You can dynamically set the amount
...
//             className="mt-4 bg-blue-600 text-white px-6 py-3 rounded-md text-lg font-bold"
//           >
//             Pay for MRI Report
//           </button>
...
const UploadMriFile = () => {
  ...
  // This function can be used for handling file upload (not being used for now)
  const handleFileUpload = (uploadedFiles) => {
    setFiles(uploadedFiles);
  };
 ...
 const handleCheckout = async () => {
   ...
   // Define the amount for the payment (e.g., $50.00)
  ...
  // Temporary function to bypass the report preview/download logic for now
  const goBack = () => {
    router.back();
  };
  ...
  return (
    <>
      ...
      {/* Temporarily hide the upload functionality for now */}
      {/* <div className="w-full max-w-4xl mx-auto min-h-80 border border-dashed bg-onHover border-gray-600 rounded-lg">
        <FileUpload onChange={handleFileUpload} />
      </div> */}

      {/* Display the Checkout button by default */}
      <div className="flex flex-col items-center p-4 my-4 rounded bg-darkslateblue text-center">
        <h3 className="text-lg font-bold text-white">Pay for MRI Report</h3>
        <p className="text-lg mt-2 text-gray-600">
          To download the MRI report, please make the payment.
        </p>

        {/* Checkout button for Stripe payment */}
        <button
          onClick={handleCheckout}
          className="mt-4 bg-blue-600 text-white px-6 py-3 rounded-md text-lg font-bold"
        >
      ...
      {/* Hide report download until payment logic is complete */}
      {/* <a
        href={`http://localhost:8800/api/mriFile${assessmentResults?.assessmentResult?.reportPath}`}
        target="_blank"
        rel="noopener noreferrer"
      ...
      {/* Removed the unnecessary buttons temporarily */}
    </>
  );
};
